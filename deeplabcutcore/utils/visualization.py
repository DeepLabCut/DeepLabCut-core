...
def MakeLabeledImage(
    ...
    if np.ndim(im) > 2:  # color image!
        ...
        h, w = np.shape(im)
    plt.figure(
        frameon=False, figsize=(w * 1.0 / 100 * scaling, h * 1.0 / 100 * scaling)
    )
...
def PlottingandSaveLabeledFrame(
    DataCombined,
    ind,
    trainIndices,
    cfg,
    ...
    if ind in trainIndices:
        full_path = os.path.join(
            foldername, "Training-" + imfoldername + "-" + imagename
        )
    else:
