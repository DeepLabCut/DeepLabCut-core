...
def Getlistofvideos(videos, videotype):
    ...
    if [os.path.isdir(i) for i in videos] == [True]:  # os.path.isdir(video)==True:
        ...
        videolist = [
            ...
            if (videotype in fn) and ("labeled.mp4" not in fn)
        ...
        if isinstance(videos, str):
            if os.path.isfile(videos):  # #or just one direct path!
                Videos = [
                    v for v in videos if os.path.isfile(v) and ("labeled.mp4" not in v)
                ]
            else:
                Videos = []
        else:
...
def LoadVideoMetadata(dataname):
    ...
    with open(
        dataname.split(".h5")[0] + "includingmetadata.pickle", "rb"
    ) as f:  # same as in SaveData!
...
def listfilesofaparticulartypeinfolder(a_dir, afiletype):
    ...
    return [name for name in os.listdir(a_dir) if afiletype in name]
...
def GetScorerName(cfg, shuffle, trainFraction, trainingsiterations="unknown"):
    ...
    if trainingsiterations == "unknown":
        ...
        modelfolder = os.path.join(
            cfg["project_path"],
            str(GetModelFolder(trainFraction, shuffle, cfg)),
            "train",
        )
        Snapshots = np.array(
    ...
    if os.path.isfile(outdataname):  # was data already processed?
        ...
        if os.path.isfile(odn):  # was it processed by DLC <2.1 project?
            ...
            return (
                False,
                odn,
                os.path.join(folder, vname + DLCscorerlegacy + suffix + ".h5"),
                DLCscorerlegacy,
            )
...
def LoadAnalyzedData(videofolder, vname, DLCscorer, filtered):
    if filtered == True:
        try:
            ...
            metadata = LoadVideoMetadata(
                os.path.join(videofolder, vname + DLCscorer + ".h5")
            )
            datafound = True
            suffix = "_filtered"
    ...
    try:  # TODO: Check if DLCscorer is correct? (based on lookup in pickle?)
        ...
        datanames = [
            fn
            for fn in os.listdir(os.curdir)
            if (vname in fn) and (".h5" in fn) and ("resnet" in fn or "mobilenet" in fn)
        ]
        if len(datanames) == 0:
            ...
            if "DeepCut_resnet" in datanames[0]:  # try the legacy scorer name instead!
                DLCscorer = (
                    "DeepCut" + (datanames[0].split("DeepCut")[1]).split(".h5")[0]
                )
            else:
