...
def create_pretrained_human_project(
    ...
    """
    print(
        "LEGACY FUNCTION will be deprecated.... use  deeplabcutcore.create_pretrained_project(project, experimenter, videos, model='full_human', ..) in the future!"
    )
...
def create_pretrained_project(
    ...
    if model in globals()["Modeloptions"]:
        ...
        cfg = deeplabcutcore.create_new_project(
        ...
        if createlabeledvideo == True:
            ...
            deeplabcutcore.create_labeled_video(
                cfg, [video_dir], videotype, draw_skeleton=True, filtered=filtered
            )
            deeplabcutcore.plot_trajectories(
                cfg, [video_dir], videotype, filtered=filtered
            )
