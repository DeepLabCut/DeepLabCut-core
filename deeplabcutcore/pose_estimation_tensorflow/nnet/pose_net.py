    ...
    with slim.arg_scope(
        ...
        with tf.compat.v1.variable_scope(name):
            pred = slim.conv2d_transpose(
                ...
                num_outputs,
                kernel_size=[3, 3],
                stride=cfg.deconvolutionstride,
                scope="block4",
            )
            return pred
        ...
        if int(vers[0]) == 1 and int(vers[1]) < 4:  # check if lower than version 1.4.
            with slim.arg_scope(resnet_v1.resnet_arg_scope(False)):
                net, end_points = net_fun(
                    im_centered, global_pool=False, output_stride=self.cfg.output_stride
                )
        else:
        ...
        with tf.variable_scope("pose", reuse=reuse):
            ...
            if cfg.intermediate_supervision:
                if (
                    ...
                    and cfg.intermediate_supervision_layer > 6
                ):
                    print(
                        "Changing layer to 6! (higher ones don't exist in block 3 of ResNet 50)."
                    )
                ...
                out["part_pred_interm"] = prediction_layer(
                    cfg, block_interm_out, "intermediate_supervision", cfg.num_joints
                )
