...
def ComputeDeviations(
    ...
    for bp in tqdm(comparisonbodyparts):
        if bp in cfg["bodyparts"]:  # filter [who knows what users put in...]
            ...
            if (
                storeoutput == "full"
            ):  # stores both the means and the confidence interval (as well as the summary stats below)
            ...
            pdindex = pd.MultiIndex.from_product(
                [[scorer], [bp], ["distance", "significant"]],
                names=["scorer", "bodyparts", "coords"],
            )
            ...
            if bpindex == 0:
                data = pd.DataFrame(
                    np.hstack([distance[:, np.newaxis], significant[:, np.newaxis]]),
                    columns=pdindex,
                )
            else:
                item = pd.DataFrame(
...
def merge_datasets(config, forceiterate=None):
    ...
    allfolders = [
        os.path.join(bf, fn) for fn in os.listdir(bf) if "_labeled" not in fn
    ]  # exclude labeled data folders!
    flagged = False
